...
class TradeStatusRtEvent:
    ...
    def OnReceived(self):
        ...
        if trade_info.e_modify_cancel_type == MODIFY_CANCEL_TYPE.MODIFY and trade_info.qty == 0:
            ...
            trade_info.qty = db_kr_operation_data.select("KR_Unconcluded_Order", "quantity", "order_number = " + str(trade_info.origin_order_num))
        ...
        if not (trade_info.e_conclusion_type == CONCLUSION_TYPE.RECEIVED and not trade_info.e_modify_cancel_type.NONE):
        ...
        if trade_info.e_conclusion_type == CONCLUSION_TYPE.CONCLUDED:
            BalanceData.change_stock_balance(trade_info.stock_code, trade_info.balance_qty, trade_info.able_sell_qty, trade_info.avg_price)
